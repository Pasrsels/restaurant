...
class StoresPersonAccessMiddleware:
    """
    Middleware to restrict stores person access to only production-related pages.
    """
    
    def __init__(self, get_response):
        self.get_response = get_response
        # Define allowed URLs for stores person (same as chef)
        self.stores_person_allowed_urls = [
            # Production Plans
    ...
    def __call__(self, request):
        # Check if user is authenticated and is a stores person
        if hasattr(request, 'user') and request.user.is_authenticated and request.user.role == 'stores_person':
            # Treat stores person the same as chef - allow all access
            # No restrictions for stores person, they have the same permissions as chef
            pass
